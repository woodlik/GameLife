...
class StateManager {
    ...
    render() {
        ...
        <br><br>
        Width: <input type="number" data-size="width" value="${
            this.options.width
        }" step="1" min="0" max="30">
        Height: <input type="number" data-size="height" value="${
            this.options.height
        }" step="1" min="0" max="20">
        <br><br>
        Speed: <input type="range" id="speed" min="${$$.min}" max="${
                $$.max
    ...
    subscribeToClick() {
        this.container.addEventListener("click", ev => {
            ...
            if (ev.target.matches("#previous")) {
                let index =
                    this.stateIndex > 0 ? --this.stateIndex : this.stateIndex;
                this.eventBus.trigger(
                    "stateManager:stateChanged",
                    this.states[index]
                );
    ...
    subscribeToChangeCell() {
        this.eventBus.on("view:cellChanged", ({ row, column }) => {
            this.gameLife.state = this.states[this.stateIndex];
            this.gameLife.state[row][column] = !this.gameLife.state[row][
                column
            ];
            this.eventBus.trigger(
                "stateManager:stateChanged",
