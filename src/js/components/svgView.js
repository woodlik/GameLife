...
class SvgView {
        ...
        for (let i = 0; i < this.state.length; i++) {
            for (let j = 0; j < this.state[i].length; j++) {
                if (this.state[i][j]) {
                    ...
                    rectangles += `<rect x='${x}' y='${y}' width='${
                        $$.squareSize
                    }' height='${$$.squareSize}' class='svg-square'/>`;
                }
            }
        }
    ...
    changeSvgSizeAccordingState() {
        this.svg.setAttribute(
            "width",
            this.state[0].length * this.options.squareSize
        );
        this.svg.setAttribute(
            "height",
            this.state.length * this.options.squareSize
        );
    }
    ...
    subscribeToClick() {
        this.svg.addEventListener("click", ev =>
            this.eventBus.trigger(
                "view:cellChanged",
                this.getCellByCoordinates(ev.offsetX, ev.offsetY)
            )
        );
    }
